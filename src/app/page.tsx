  ...
  return (
    <div className="font-sans grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20">
      <main className="flex flex-col gap-[32px] row-start-2 items-center sm:items-start w-full max-w-xl">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="w-full">
          <h2 className="text-lg font-semibold mb-2">Zustand Counter</h2>
          <div className="flex items-center gap-2">
            <button onClick={decrement} className="px-3 py-1 rounded border hover:bg-gray-100 dark:hover:bg-gray-800">
              -
            </button>
            <span className={clsx("w-12 text-center font-mono", count % 2 === 0 ? "text-blue-600" : "text-purple-600")}>{count}</span>
            <button onClick={increment} className="px-3 py-1 rounded border hover:bg-gray-100 dark:hover:bg-gray-800">
              +
            </button>
            <button onClick={reset} className="px-3 py-1 rounded border hover:bg-gray-100 dark:hover:bg-gray-800">
              Reset
            </button>
          </div>
        </motion.div>
